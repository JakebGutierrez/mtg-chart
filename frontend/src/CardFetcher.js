...
function CardFetcher() {
  ...
  function handleDrop(event, index) {
    ...
    if (source === 'searchResult') {
      ...
      setSearchResults((prev) =>
        prev.filter((card) => card.id.toString() !== cardId)
      )
    } else if (source === 'gridItem') {
      const sourceIndex = gridItems.findIndex(
        (item) => item && item.id.toString() === cardId
      )
  ...
  const loadFromLocalStorage = useCallback(() => {
    try {
      ...
      const lastOpenedChart = savedCharts.find(
        (chart) => chart.id === lastOpenedChartId
      )
            ...
            return (
              ...
              >
                {currentCard &&
                  ...
                  currentCard.image_uris.art_crop && (
                    <img
